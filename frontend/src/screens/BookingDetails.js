...
import { cancelBooking } from '../redux/actions/hotelAction';
...
const BookingDetails = () => {
    ...
    const handleSubmit = () => {
        if (window.confirm('Bạn có chắc chắn muốn hủy phòng?')) {
            dispatch(cancelBooking('Đã hủy',booking.id)); // Gọi action để hủy phòng
        }
    }
    ...
    return (
        <Fragment>
            ...
            <button
                ...
                onClick={handleSubmit} // Gọi hàm handleSubmit khi nhấp vào nút
            >
                Hủy phòng
            </button>
        </div>
