...
export const cancelBooking = (status, bookingId) => async (dispatch) => {
    try {
        const { data } = await axios.put(`/api/v1/me/bookings/${bookingId}/cancel`, { status }, { headers: { "Content-Type": "application/json" } });
        dispatch(setAllBookings(data.bookings));
    } catch (err) {
        dispatch(setError(err.response.data.message));
    }
